...
foreach ($cd in $cdInfo) {
    ...
    if ( ($diff.TotalMinutes -ge $bugMe) -and ($diff.TotalMinutes -le $cd.alertTime) ) {
        ...
        if ( ($cd.keepBuggingMe -eq $True) -and ($diff -le 0) -and ($cd.interval -eq $True) -and ($diff.TotalMinutes%$cd.intervalTime -le 10)) {
        ...
        elseif ( ($cd.interval -eq $True) -and ($diff.TotalMinutes%$cd.intervalTime -le 10) ) {
